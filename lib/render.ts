...
export function render(calendar:Calendar,o:Options,textRenderer?:TextRenderer) {
  ...
  const step=Math.min(maxW/cols,maxH/rows)*o.scale,size=step*.76,gw=cols*step,gh=rows*step,x0=(w-gw)/2,naturalY=(h-gh)/2-(portrait?0:h*.03),safeTop=h*(portrait?.25:.12),safeBottom=h*.84-Math.min(w*.028,h*.035)*(o.avatar?7:6)-gh,y0=legacy?naturalY:Math.max(safeTop,Math.min(safeBottom,naturalY+(o.position-.5)*h*.35));
  ...
  const avatarSize=font*1.5, identityY=bottom+font*(o.avatar?4:3), nameSize=font*.75;
  ...
  const footerBackdrop=legacy?'':`<rect x="${w*.12}" y="${bottom-font*1.8}" width="${w*.76}" height="${font*(o.avatar?6.5:5.5)}" rx="${font*.7}" fill="${ink==='#182027'?'#f3f5f8':'#101820'}" opacity=".78"/>`;
